...
start:                
    ...
    NUMBER_ODD:                     ; если число нечетное
         MOV DI, [index_arrar_odd]  ; записываем в DI значение индекса массива для нечетных
         MOV BP, [addres_array_odd] ; записываем в BP адресс массива для нечетных 
         MOV [BP+DI], AX            ; сохранение нечетного числа в массиве   
         ADD SI, 2                  ; переход к следующему элементу массива
         INC [count_odd]            ; увеличение количества нечетных чисел  
         ADD [index_arrar_odd], 2   ; увеличиваем индекс след значения массива для нечетных
    LOOP OUTPUT                     ; переходим в метку OUTPUT ЕСЛИ CX != 0
    JCXZ NEXT_ARRAY_SORT        ; если CX = 0, то идем дальше
      
    NUMBER_EVEN:                     ; если число четное
         MOV DI, [index_array_even]  ; записываем в DI значение индекса массива для четных
         MOV BP, [addres_array_even] ; записываем в BP адресс массива для четных   
         MOV [BP+DI], AX             ; сохранение четного числа в массиве   
         ADD SI, 2                   ; переход к следующему элементу массива
         INC [count_even]            ; увеличение количества четных чисел  
         ADD [index_array_even], 2   ; увеличиваем индекс след значения массива для четных
    LOOP OUTPUT                      ; переходим в метку OUTPUT ЕСЛИ CX != 0 
    JCXZ NEXT_ARRAY_SORT         ; если CX = 0, то идем дальше
